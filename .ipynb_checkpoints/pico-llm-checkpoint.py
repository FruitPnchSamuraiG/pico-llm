...
class KGramMLPSeqModel(nn.Module):
    ...
    def __init__(self, vocab_size, k=3, embed_size=1024, num_inner_layers=1, chunk_size=1):
        ...
        # The input size is (k * vocab_size) because each of the last k tokens
        # is represented as a one-hot vector of length vocab_size.
        layers = []
        in_dim = k * vocab_size
        out_dim = embed_size
        ...
        # Add any intermediate hidden layers (if num_inner_layers > 1)
        ...
        # Combine all into a sequential MLP
...
class TransformerModel(nn.Module):
    def __init__(self, vocab_size=50257, d_model=1024, n_heads=2, n_blocks=4, block_size=128):
        super().__init__()
    ...
    def forward(self, idx):
        ...
        pos_emb = self.position_embedding(torch.arange(t, device=idx.device))[None, :, :]
        x = tok_emb + pos_emb
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x):
        y = self.attn(self.rmsnorm1(x), self.rmsnorm1(x), self.rmsnorm1(x))[0]
        x = x + y
        y = self.mlp(self.rmsnorm2(x))
        x = x + y
        return x
...
def nucleus_sampling(logits, p=0.95):
    ...
    Args:
        logits (Tensor): unnormalized logits over vocab (vocab_size,)
        p (float): cumulative probability threshold (e.g., 0.9 or 0.95)

    Returns:
        int: index of the chosen token
    """
    # Convert logits -> probabilities (numerically stable)
    ...
    if torch.any(cutoff):
        cutoff_idx = torch.nonzero(cutoff, as_tuple=False)[0, 0] + 1
    else:
    ...
    # Renormalize to make probabilities sum to 1
    top_probs = top_probs / top_probs.sum()

    # Sample one token from this filtered distribution
...
def main():
    ...
    for model_name, model in models.items():
        ...
        train_loss, val_loss = train_one_model(
            model=model,
            loader=train_loader,
            epochs=num_epochs,
    ...
    plot_loss_curves(all_results, save_path="loss_curves.png")
